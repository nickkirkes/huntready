...
**M1 in progress (E01 complete; E02 active — 7/8 stories complete).** M0 scaffold complete. E01 (schema migrations, RLS, quality gates) merged 2026-04-28: 10 tables (7 entity + 3 link) created in `supabase/migrations/`, deny-all RLS active on every table with three-layer defense-in-depth (ENABLE+FORCE, deny-all policies, REVOKE), Pydantic models in `ingestion/ingestion/lib/schema.py` and TypeScript interfaces in `mcp-server/src/types/` mirror the DDL, pre-commit hooks installed. Migration reproducibility verified against fresh Supabase projects; runbook at `docs/runbooks/E01-migration-verification.md`. E02 S02.0 (schema prep) merged 2026-04-29: `geometry.verbatim_rule text` (nullable) added; `SourceCitation.document_type` extended with `'gis_layer'` per ADR-014. E02 S02.1 (ArcGIS fetch infrastructure shared library) merged 2026-04-29. E02 S02.2 (Hunting District ingestion) merged 2026-04-30: 235 Montana HDs loaded; named multi-part HD anchor `MT-HD-deer-elk-lion-690-geom` (12 parts) reserved for S02.7. E02 S02.3 (Portions ingestion) merged 2026-04-30: 55 Montana portions loaded. E02 S02.4 (Restricted Areas with verbatim text) merged 2026-04-30: 57 restricted-area geometries; 55 of 57 rows exercise the ADR-015 separator branch. E02 S02.5 (CWD zone discovery and ingestion) merged 2026-05-01: 2 CWD zones loaded via the dedicated `ADMBND_HD_CWD` FeatureServer on ArcGIS Online; total geometry rows: 349. E02 S02.6 (Geometry overlay fixture) merged 2026-05-01: built `ingestion/states/montana/fixtures/geometry-overlays.json` plus a committed audit log at `geometry-overlays-dropped.json`; computation moved off SQL entirely (Supabase's 2-min `statement_timeout` aborts cross-joins on real data) to local shapely + STRtree (~5 sec end-to-end); ADR-016 codifies the three-band area-ratio discriminator (`COVER_RELABEL_THRESHOLD=0.99`, `COVER_DROP_THRESHOLD=0.01`, asymmetric `child.area` denominator) since strict `ST_Covers` rejects 32 of 55 Montana portions due to digitization precision; per-ID `EXPECTED_RA_ORPHAN_IDS` allowlist tolerates 3 documented no-hunt zones (Glacier NP, Sun River Game Preserve, Yellowstone NP). S02.7 (Spatial query verification + epic exit, including the small `arcgis.fetch_features` manifest extension) is next.
