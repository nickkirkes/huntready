...
**M1 in progress (E01 complete; E02 active — 5/8 stories complete).** M0 scaffold complete. E01 (schema migrations, RLS, quality gates) merged 2026-04-28: 10 tables (7 entity + 3 link) created in `supabase/migrations/`, deny-all RLS active on every table with three-layer defense-in-depth (ENABLE+FORCE, deny-all policies, REVOKE), Pydantic models in `ingestion/ingestion/lib/schema.py` and TypeScript interfaces in `mcp-server/src/types/` mirror the DDL, pre-commit hooks (TypeScript lint + Python ruff + secrets scanning) installed. Migration reproducibility verified against fresh Supabase projects; runbook at `docs/runbooks/E01-migration-verification.md`. E02 S02.0 (schema prep) merged 2026-04-29: `geometry.verbatim_rule text` (nullable) added; `SourceCitation.document_type` extended with `'gis_layer'` (type-layer enforcement per ADR-014). E02 S02.1 (ArcGIS fetch infrastructure shared library) merged 2026-04-29. E02 S02.2 (Hunting District ingestion) merged 2026-04-30: 235 Montana HDs loaded; named multi-part HD anchor `MT-HD-deer-elk-lion-690-geom` (12 parts) reserved for S02.7. E02 S02.3 (Portions ingestion) merged 2026-04-30: 55 Montana portions loaded with layer-wide SHAPECODE→PORTIONNAME slug fallback handling real layer-#12 collisions, plus zero-features-fail-loud guard. E02 S02.4 (Restricted Areas with verbatim text) merged 2026-04-30: 57 restricted-area geometries loaded — 53 from layer #2 (license_year=NULL, no REGYEAR), 4 from layer #15 (license_year=2026); 55 of 57 rows exercise the ADR-015 five-case `\n\n--- COMMENTS ---\n\n` separator branch. Shared CWD discriminator at `ingestion/states/montana/cwd_discriminator.py` imported by S02.4 (inverse filter) and pre-staged for S02.5; live load surfaced empirical finding that layer #2 has zero CWD matches, so S02.5 must use the Hub-catalog or hand-traced fallback paths. S02.5 (CWD zone discovery and ingestion) is next.
