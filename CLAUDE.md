...
**M1 in progress (E01 complete; E02 active — 4/8 stories complete).** M0 scaffold complete. E01 (schema migrations, RLS, quality gates) merged 2026-04-28: 10 tables (7 entity + 3 link) created in `supabase/migrations/`, deny-all RLS active on every table with three-layer defense-in-depth (ENABLE+FORCE, deny-all policies, REVOKE), Pydantic models in `ingestion/ingestion/lib/schema.py` and TypeScript interfaces in `mcp-server/src/types/` mirror the DDL, pre-commit hooks (TypeScript lint + Python ruff + secrets scanning) installed. Migration reproducibility verified against fresh Supabase projects; runbook at `docs/runbooks/E01-migration-verification.md`. E02 S02.0 (schema prep) merged 2026-04-29: `geometry.verbatim_rule text` (nullable) added; `SourceCitation.document_type` extended with `'gis_layer'` (type-layer enforcement per ADR-014). E02 S02.1 (ArcGIS fetch infrastructure shared library) merged 2026-04-29: state-agnostic `ingestion/ingestion/lib/arcgis.py` with metadata-driven pagination, projection guards, error-envelope retries, per-host throttling, source-fixture write path, and `compute_feature_hash` for change detection. E02 S02.2 (Hunting District ingestion) merged 2026-04-30: 235 Montana HDs loaded — 61 antelope (#3), 35 black bear (#10), 139 deer-elk-lion (#11) — into `geometry` table with species-prefixed deterministic IDs and per-feature SourceCitations; named multi-part HD anchor `MT-HD-deer-elk-lion-690-geom` (12 parts) reserved for S02.7. E02 S02.3 (Portions ingestion) merged 2026-04-30: 55 Montana portions loaded — 4 antelope (#4), 11 mule deer (#12), 13 whitetail (#13), 27 elk (#14) — using `kind='portion'`, layer-wide slug strategy (SHAPECODE preferred, slugified PORTIONNAME fallback) that handles real SHAPECODE collisions surfaced in layer #12, and zero-features-fail-loud guard. S02.4 (Restricted Areas with verbatim text on layers #2 and #15) is next.
