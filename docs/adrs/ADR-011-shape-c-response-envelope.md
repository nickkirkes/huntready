...
**M3 refinement (2026-06-30, E09).** Added a sixth always-present null-bearing section, **`additional_rules: AdditionalRulesSection | null`** (`{ rules: VerbatimRule[]; source }`), surfacing the HD-wide verbatim prose on `RegulationRecord.additional_rules` (DEA `NOTE:` lines, the statewide Bear ID Test rule) not structured by a season/tag/reporting child entity. Without it, verbatim regulatory text the corpus loaded would be silently dropped at the serving layer — an ADR-001/ADR-008 gap. Each `VerbatimRule` carries its own `confidence` (one of the two confidence carriers per ADR-017 §3) and is surfaced byte-identically (ADR-008). `Warning.section` gains `"additional_rules"`. See `architecture.md` §"Response shape".
