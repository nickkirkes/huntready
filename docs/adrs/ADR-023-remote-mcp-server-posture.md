...
Build the M3 MCP server as a **remote, spec-conformant Streamable HTTP server deployed on Cloudflare Workers** (stateless `createMcpHandler`; no Durable Objects, since the tools are stateless reads), retaining a stdio entrypoint for local development via the `mcp-remote` proxy. Keep V1 auth **OAuth-2.1-ready but unenforced**: the endpoint is a **single open, read-only MCP endpoint** (public data; the M4 web app calls it directly with **no BFF**), with **no enforced authentication** in V1. The OAuth-2.1 auth seam is **wired as one middleware integration point but not enforced** — the drop-in for real auth (`@cloudflare/workers-oauth-provider`) when a GTM model lands (Q22). No static bearer token is relied on as a V1 boundary (a single open endpoint can't enforce a client-type split — a client could omit the token and take the open path; and a browser-shipped token is exposed anyway). Baseline abuse protection is Cloudflare's **ambient DDoS/WAF**; any *enforced* token, tier, or quota is V2 and needs a real boundary (a separate authenticated route or Cloudflare Access). This resolves Q5 (no BFF) and Q6 (Cloudflare Workers).
...
Auth is deliberately minimal — **wired but unenforced** — because a token gate on a V1 public endpoint would be illusory. Two traps: the no-BFF browser-direct path (Q5) cannot hold a secret (a key shipped to the browser is exposed in client-side code), and on a *single open endpoint* a token can't be enforced anyway (a client could omit it and take the open path). So V1 ships an open, read-only endpoint with no enforced auth, the OAuth seam wired as the drop-in point, and Cloudflare's ambient DDoS/WAF as baseline protection. The data is public hunting regulations — there is no per-user resource owner — so this matches the standing "no authentication in V1" scope. Any enforced auth/tier/quota is V2 and requires a real boundary (a separate authenticated route or Cloudflare Access); the full OAuth 2.1 flow drops in when a go-to-market strategy gives the auth model a subject to protect (deferred as Q22).
...
- Spec-conformant remote, auth-ready MCP server (the seam is wired for OAuth even though V1 leaves it unenforced) — the recognizable reference pattern (Linear, the Cloudflare MCP cohort) and the stronger portfolio/production signal.
