...
Build the M3 MCP server as a **remote, spec-conformant Streamable HTTP server deployed on Cloudflare Workers** (stateless `createMcpHandler`; no Durable Objects, since the tools are stateless reads), retaining a stdio entrypoint for local development via the `mcp-remote` proxy. Gate the deployed endpoint with a **minimal static bearer-token / API-key checkpoint** (OAuth-2.1-ready, via `@cloudflare/workers-oauth-provider` as the documented upgrade path). The M4 web app calls the server directly with **no BFF**. This resolves Q5 (no BFF) and Q6 (Cloudflare Workers).
...
Auth is deliberately minimal but ready. The data is public hunting regulations — there is no per-user resource owner — so the V1 gate's real job is access metering and abuse-control, not authorization. A static bearer-token checkpoint behind a single seam delivers that today; the full OAuth 2.1 flow drops in later when a go-to-market strategy gives the auth model a subject to protect (deferred as Q22).
