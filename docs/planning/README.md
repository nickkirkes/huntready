...
**Current Milestone:** M1 — Montana Ingestion (E01 complete, E02 active — S02.0/S02.1/S02.2/S02.3/S02.4/S02.5 complete, S02.6 next)
...
| E02 | Montana Geometry Ingestion | In Progress (6/8 stories complete) | 2026-04-28 | — | 8 |
...
None blocking S02.6. S02.5 (CWD zone discovery and ingestion) merged 2026-05-01 with **Path A — standalone GIS layer found** via dedicated `ADMBND_HD_CWD` FeatureServer on ArcGIS Online; 2 CWD zones loaded (Libby, Kalispell). All four V1 geometry kinds now present in the database: hunting_district (235), portion (55), restricted_area (57), cwd_zone (2) = **349 total geometry rows ready for S02.6's overlay fixture**.

One PRD-reconciliation item remains, but it does **not** block S02.6:
...
- Begin S02.6 (Geometry overlay fixture). PostGIS `ST_Covers` and `ST_Intersects` queries on the `geometry` table compute HD↔Portion (containment), HD↔CWD zone (containment/intersection), and HD↔Restricted Area (containment/intersection) relationships, plus self-referential primary_unit rows for every hunting_district. Output to `ingestion/states/montana/fixtures/geometry-overlays.json` with `role_for_e03` pre-computed so E03 can directly populate `jurisdiction_binding` rows once `regulation_record` rows exist. Coverage invariant strengthened to per-kind (every HD has self-relationship; every portion/cwd_zone/restricted_area appears as child of an HD).
