...
**Current Milestone:** M1 — Montana Ingestion (E01 + E02 complete; E03 active — 10 of 13 original stories complete + 1 new placeholder (S03.6.1) added during S03.9's source-audit probe: S03.0-S03.8 (see history) + S03.9 reporting_obligation + regulation_reporting ingestion (closed 2026-05-21; 3 reporting_obligation rows + 70 regulation_reporting link rows; Q18 + Q19 opened). Total stories on the M1 path: **14** (10 done + S03.6.1 + S03.10 + S03.11 + S03.12).)
...
| E03 | Montana Regulation Text Ingestion | In Progress (10/14 — S03.9 closed 2026-05-21; S03.6.1 + S03.10 + S03.11 + S03.12 remain; Q18 + Q19 opened) | 2026-05-03 | — | 14 (13 original + S03.6.1) |
...
| S03.6.1 | MT-STATEWIDE-bear anchor with Bear ID Test + jurisdiction_binding to MT-STATEWIDE-geom | Not Started — queued post-S03.9 close; spec hardened 2026-05-21 | Implementation (carved out 2026-05-19; binding scope-add 2026-05-21) |
...
- **Begin S03.6.1 (MT-STATEWIDE-bear anchor + jurisdiction_binding to MT-STATEWIDE-geom) — UNBLOCKED; spec hardened 2026-05-21.** Carved out 2026-05-19 from S03.9's source-audit probe; binding scope-add accepted 2026-05-21 to keep the statewide-anchor entity self-contained at story close. Adds **1 new `regulation_record`** (`jurisdiction_code='MT-STATEWIDE-bear'`, `species_group='bear'`, Bear ID Test in `additional_rules`) + **1 new `jurisdiction_binding`** (`geometry_id='MT-STATEWIDE-geom'`, `role='primary_unit'`) in one atomic transaction. Introduces the **`db.upsert_jurisdiction_binding` helper** that S03.10 then reuses for its broader overlay-derived binding generation; mirrors the S03.6 pattern of introducing `update_legal_description` ahead of its S03.8 reuse. **Upstream change**: extend `extract_black_bear.py` page-2 right-column bbox + 2 regex anchors. **Adapter change**: amend `load_regulation_records.py` for both the new regulation_record and the new jurisdiction_binding. **Row counts**: regulation_record 436→437 (`_REGULATION_RECORD_COUNT_GUARD` widens to `[305, 568]`); jurisdiction_binding 0→1 (new `_JURISDICTION_BINDING_COUNT_GUARD` band `[1, 1]` since this story writes exactly one binding; S03.10 broadens later). **Pattern reference**: `MT-STATEWIDE-antelope`'s additional_rules pattern (verified 2026-05-20 — populated today via S03.6 NOTE-line extraction). Antelope's `jurisdiction_binding` does NOT yet exist; S03.10 derives it symmetrically. **`id` encoding for jurisdiction_binding must be deterministic** (recommended `f"{state}-{jurisdiction_code}-{species_group}-{license_year}-{role}-{geometry_id}"`); S03.10 reuses the same encoding so UPSERT-by-id makes S03.10's later derivation a harmless no-op.
- **After S03.6.1: S03.10 (jurisdiction_binding generation, UAT-yes)** — load-bearing binding-generation story that ties everything together. Per epic depends on S03.6 only (not S03.7/S03.8/S03.9), but S03.6.1 should land first because it ships the `db.upsert_jurisdiction_binding` helper + the binding-id encoding S03.10 inherits. Q18 (CWD modeling) MAY affect S03.10's CWD-overlay binding logic — confirm before binding generation locks in zone-keyed assumptions.
