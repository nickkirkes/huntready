...
**Current Milestone:** M2 — Colorado Ingestion (In Progress; E04 active with 4 of 5 stories closed — S04.2 closed 2026-05-29; S04.1 fully closed 2026-05-30 (Group A at-merge + Group B live-verified); S04.3 closed 2026-05-30 via PR #49 (`5de83c3`); S04.4 closed 2026-05-30 (fourth M2 PR; M1 UAT runbook hygiene fixes per handoff §8 #1-#6 + mandatory criterion #7 sign-off annotation + PM-approved adopted 7th row for `jurisdiction_binding 788 build → 788 DB` completing the build-vs-DB convention; draw_spec 278-vs-276 provenance reconciled per S03.8 closure; user-approved AC-interpretation override for §6 audit-trail preamble; three pitfall candidates surfaced for user decision). **S04.5 (PRD 001 sequencing language reconciliation) is the only remaining E04 story and is human-action-gated** — PM drafted the diff at E04 open; user applies to PRD 001 directly. E05 and E06 planned later via `/plan-next-epic`). M1 closed 2026-05-27 with `m1` tag at PR #45 (`ccbe085`).
...
| M2 | Colorado Ingestion | In Progress (E04 active; S04.1 + S04.2 + S04.3 + S04.4 closed; S04.5 human-action-gated) | 2026-05-29 (E04) | M1 |
...
|---|---|---|---|---|---|
| E04 | M1 Carry-Forward and Colorado Schema Preparation | In Progress (4 of 5 stories closed; S04.2 closed 2026-05-29; S04.1 + S04.3 + S04.4 all closed 2026-05-30 — see story-level table for detail; S04.5 is the only remaining E04 story and is human-action-gated) | 2026-05-29 | — | 5 (S04.6 evaluated and omitted) |
...
None. M2 E04 fully closed S04.1 on 2026-05-30 — Group A satisfied at merge (PR #48 squash from `feat/S04.1-license-season-rls-migration`) and Group B closed via live verification later same day. All four verification queries against the production Supabase project returned the expected shape (Q1: 0 info-schema privileges for anon/authenticated; Q2: 2 deny-all policies attached, byte-exact policy names; Q3: `relrowsecurity` + `relforcerowsecurity` both true; Q4: 2411 rows preserved — the post-UPSERT-collapse DB baseline per S04.4 spec § "Expected counts" L280, not the ~3040 build count from handoff §3 which the verification prompt had cited in error). Verbatim outputs locked in `docs/planning/epics/E04-m1-carry-forward.md` § "Group B verification record — 2026-05-30". The M1 UAT criterion #7 leak surface (14 privilege leaks + 0 RLS policies on `license_season` confirmed at M1 UAT 2026-05-28) is now structurally closed in production. **Discipline note for future Group B-style verification prompts**: cite the post-UPSERT DB baseline (S04.4 spec L280 is the durable source-of-truth) not the build count from handoff §3 — recommended a future implementation agent land as a one-line entry in `.roughly/known-pitfalls.md`; PM does not touch `.roughly/` autonomously.

**S04.5 (PRD 001 sequencing language reconciliation) is the only remaining E04 story and is human-action-gated.** PM drafted the line-by-line diff at E04 open in S04.5's Context section (lines 48 no-change / 90 / 96 / 111); user reviews and applies to PRD 001 directly per the no-autonomous-PRD-edit rule; PM flips the checkbox on the human's merge confirmation. Status flow: `In Progress` while the diff is in the human's hands; `Complete` when the human reports PRD 001 has merged to main. The diff has been on-record since 2026-05-29 E04 open — no PM action needed; awaiting human application.

**Three M2 pitfall candidates surfaced at S04.4 close (PM cannot land autonomously; `.roughly/` is implementation territory)** — recommended for the next implementation agent or for a user-initiated `.roughly/known-pitfalls.md` edit pass:

1. **Most load-bearing** — "Spec-prescribed string substitutions silently invalidate coupled references elsewhere in the document; grep the file for every coupled reference and decide per-occurrence before review catches it." S04.4 T2/T3's HD 262 → HD 124 swap silently invalidated four downstream references caught only by Stage-6 review.
2. "Authoritative numbers drift between canonical documents — name the source-of-truth before copying" (draw_spec 278-vs-276 case; extends S04.1's build-vs-DB confusion lesson; can fold into #1).
3. "Cubic-review on documentation diffs can flag values it lacks context to validate; treat as advisory not gating" — PM recommends NOT landing (operational reality of cubic, not a recurring trap).

**Five handoff hygiene-edit candidates accumulated from M2 work** (PM cannot modify; handoff is read-only per the M2 PM prompt): (1) handoff §1/§6/§7/§9 path-drift to `completed/`; (2) handoff §3 build-vs-DB count inconsistency (~3040 build count semantics vs. S04.4's 2411 DB authoritative); (3) handoff §8 item #7 RESOLVED annotation (S04.3 / PR #49 / `5de83c3`); (4) handoff §8 item #4 transcription error (draw_spec `276` → `278` per S03.8 closure authoritative); (5) handoff §8 item #4 incompleteness (missing `jurisdiction_binding 788 build → 788 DB` row now in the shipped runbook). Recommend a consolidated single-commit patch when the user is ready.
