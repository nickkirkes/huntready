...
**Current Milestone:** M1 — Montana Ingestion (E01 + E02 complete; E03 active — 9/13 stories complete: S03.0-S03.7 (see history) + S03.8 draw_spec ingestion (closed 2026-05-19; 388 draw_spec rows / 278 unique + 388 license_tag.draw_spec_key backfills + S03.7 kind heuristic amended to reclassify 162 OTC B Licenses; Q17 per-HD-allocation-caps opened for M2))
...
| E03 | Montana Regulation Text Ingestion | In Progress (9/13 stories complete; S03.8 closed 2026-05-19 — draw_spec + S03.7 kind-heuristic amendment; Q17 for M2) | 2026-05-03 | — | 13 |
...
- **S03.8 closed 2026-05-19** via PR `323836f` (8 commits `5b48936..d733482`). Final state: **388 draw_spec rows (278 unique after UPSERT composite-PK collapse) + 388 license_tag.draw_spec_key backfills** in one atomic three-phase transaction. **S03.7 license_tag.kind heuristic amended in the same PR**: 162 over-the-counter B Licenses were previously mis-labeled `limited_draw` because OQ-S7-7 only inspected `license_code`; amendment inspects `apply_by` cross-row AND cross-HD; post-fix DEA breakdown is **388 limited_draw + 162 over_the_counter + 239 general + 1 statewide = 790 unique** (counts unchanged; classification corrected). Without this fix S03.8 would have over-written 162 OTC licenses with `draw_spec` rows. `900-20` correctly has no draw_spec; deferred-items file `draw-mechanics.md` now has 3 entries (900-20 ordering; MT NR-cap 10%; per-HD allocation caps via HD 210 case). **Q17 opened in `docs/open-questions.md`** — per-HD allocation caps in `draw_spec`; M2 ADR-candidate; V1 handles via `_KNOWN_CROSS_LISTING_OVERRIDES` override-dispatch table. **Three new patterns established for S03.9+**: (1) cross-listing consistency validator (use `"key" in dict` for presence detection, not `.get(...) is None`); (2) defensive safety-net lookup (`_DEA_DEADLINE_LOOKUP` is the prototype — fields missing per-row but present in front-matter get an adapter-level fallback that RAISES on runtime firing); (3) override-dispatch table for known-irreconcilable real-PDF inconsistencies (`_KNOWN_CROSS_LISTING_OVERRIDES` is the prototype). Four new pitfalls in `.roughly/known-pitfalls.md`. Test suite: **910 + 2 skipped** (+78 from S03.8).
- **Begin S03.9 (reporting_obligation ingestion) — UNBLOCKED.** Reads the Bear artifact's 3 candidate reporting_obligations (STATEWIDE 48-hour, R1 tooth_submission 10-day, R2-7 hide_skull_presentation 10-day) plus any DEA prose reporting (per S03.4's revised count). Writes `reporting_obligation` rows + `regulation_reporting` link rows. FK target: 436 regulation_record rows from S03.6 + S02.6's overlay fixture for CWD-zone-overlap lookup. **HD→region mapping**: Montana convention is HD-number-first-digit → R1-R7 (confirm against DEA p. 5 "How To Use These Regulations"). Inherits OQ7 row-count guards (2 expected: one for reporting_obligation, one for regulation_reporting). UAT: no per epic.
- **S03.9 can carry forward S03.8's three new patterns**: cross-listing consistency validator if any reporting_obligation has overlapping HD coverage; defensive safety-net lookup if any deadline lives in front-matter rather than per-row; override-dispatch table if any real-PDF inconsistency surfaces. Apply per discovery — only if needed.
