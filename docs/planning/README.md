...
**Last Updated:** 2026-06-01 (S05.0 closed at-merge via PR #54 `7f3b071`; test baseline shifted 1166 → **1184** + 2 skipped; S05.1/S05.2/S05.3 unblocked; 2 M2 hygiene candidates queued from S05.0 cubic-fix iterations)
...
| M2 | Colorado Ingestion | In Progress (E04 Complete + Audited 2026-05-31; **E05 In Progress** — 1 of 8 stories closed (S05.0 at-merge 2026-06-01); S05.1/S05.2/S05.3 unblocked; S05.4 awaits research-doc prerequisite; E06 awaits E05 audit) | 2026-05-29 (E04), 2026-05-31 (E05) | M1 |
...
| E05 | Colorado Geometry Ingestion | **In Progress** (1 of 8 stories closed; S05.0 at-merge 2026-06-01 via PR #54 `7f3b071`; test baseline shifted 1166 → **1184** + 2 skipped; S05.1/S05.2/S05.3 unblocked) | 2026-05-31 | — | 8 (S05.0–S05.7) |
...
**Active work-front: E05 (Colorado geometry ingestion).** Planned + validated 2026-05-31; pre-implementation review closed 2026-05-31 (PR #53 / `fd989b0`); **S05.0 closed at-merge 2026-06-01** (PR #54 / `7f3b071`) — source = US Census TIGER 2024 Tier-3 fallback per source-priority framing; 9 fail-loud guards in loader; 4 new pitfalls landed; test baseline shifted 1166 → **1184** + 2 skipped via +18 new unit tests (deliberate quality addition); Group A satisfied at-merge; Group B operator-pending pending live `supabase db push` + loader-run (mirrors S04.1's PRD-006-style operator-verifies-live pattern). **Three stories now unblocked** per E05 epic §"Parallelization Notes" — S05.1 (CO adapter scaffold), S05.2 (GMU ingestion), S05.3 (CWD-zone discovery + ingestion) — can begin in any order; convention is sequential per the recommended merge order. **S05.4 remains hard-blocked** on `docs/research/colorado-restricted-areas-evaluation.md` research-doc prerequisite (human or research-drafting session deliverable; PM does NOT draft autonomously). E06 planned later via `/plan-next-epic` after E05's audit closes.
...
**M2 hygiene candidates queued from S05.0 close (PM cannot land autonomously; `ingestion/` is implementation territory)** — flagged to user for decision:

1. **`db.upsert_geometry` missing `cur.rowcount == 0` fail-loud guard** — inconsistent with the `update_legal_description` / `update_license_tag_draw_spec_key` / `upsert_jurisdiction_binding` pattern established post-S03.6. Low risk today (psycopg3 returns `rowcount=1` for both INSERT and UPDATE under ON CONFLICT DO UPDATE); risk surfaces if DDL ever changes to ON CONFLICT DO NOTHING. Single-PR fix in `ingestion/ingestion/lib/db.py`.
2. **MT `load_state_boundary.py --service-url` provenance silent-lie** — same latent issue cubic-fixed in CO during S05.0 Iter 1 P2. Single-line removal + test-import update.

Both are small, isolated, and would land in a focused hygiene PR. If you want either folded into S05.1 (CO adapter scaffold), now is the moment to decide; otherwise they queue for whenever the hygiene window opens.
