...
| S03.6.1 | MT-STATEWIDE-bear anchor with Bear ID Test | Not Started — queued post-S03.9 close | Implementation (carved out 2026-05-19 from S03.9 source-audit probe) |
...
- **Begin S03.6.1 (MT-STATEWIDE-bear anchor with Bear ID Test) — UNBLOCKED. Carved out 2026-05-19 from S03.9's source-audit probe.** Adds 1 new `regulation_record` row (`jurisdiction_code='MT-STATEWIDE-bear'`, `species_group='bear'`) with `additional_rules` carrying the Bear ID Test pre-purchase prerequisite text (already extracted; on page 2 of bear booklet right column). **Upstream change**: extend `extract_black_bear.py` page-2 right-column bbox + 2 regex anchors. **Adapter change**: amend `load_regulation_records.py` to recognize the new artifact field. **Row count**: 436 → 437; update `_REGULATION_RECORD_COUNT_GUARD` band. **Pattern reference**: `MT-STATEWIDE-antelope` (verified 2026-05-20: its `additional_rules` IS populated today with 2 NOTE entries; S03.6.1 is pattern extension, not pure replay). **Coordination question for S03.10**: does S03.6.1 need a corresponding `jurisdiction_binding` to `MT-STATEWIDE-geom`? Likely yes (mirrors antelope binding). Decide before S03.10 ships or include in S03.6.1.
- **After S03.6.1: S03.10 (jurisdiction_binding generation, UAT-yes)** — the load-bearing binding-generation story that ties everything together. Per epic line 1144 depends on S03.6 only (not S03.7/S03.8/S03.9), so technically unblocked, but S03.6.1 should land first to avoid a follow-on patch. Q18 (CWD modeling) MAY affect S03.10's CWD-overlay binding logic — confirm before binding generation locks in zone-keyed assumptions.
