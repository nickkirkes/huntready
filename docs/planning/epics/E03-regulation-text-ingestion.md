...
**Status:** In Progress (3/13 stories complete; S03.3 RE-OPENED 2026-05-08 after UAT failure)
...
**UAT Gating:** S03.3 (UAT FAILED 2026-05-08; six defects; fix directive at `E03-confidence-findings/S03.3-uat-fixes.md`), S03.4, S03.5 (per-booklet faithfulness review), S03.7, S03.10, S03.12 (entity ingestion + binding + milestone exit)
...
- [ ] **`season_windows[<key>]` carries both `window` (date span string) and `weapon_type_override`** derived from source column header, not season name. **RE-OPENED 2026-05-08 by UAT failure:** current implementation uses section-level first-observation-wins, replicating row 1's window across all rows in the section. PDF rows have per-row date variation (HD 124 deer alone has 3 distinct General windows); affects ≥143 multi-row sections. Fix directive: [`E03-confidence-findings/S03.3-uat-fixes.md`](E03-confidence-findings/S03.3-uat-fixes.md) § Fix 1.
- [ ] All antelope HD subsections (pp. 136-141 — actual PDF is 141 pages, not 142 as spec claimed) extracted, plus the `900-20` statewide overlay row. **RE-OPENED 2026-05-08 by UAT failure:** Region 7 "Portions of HDs ..." sub-sections are entirely missing from the artifact; their rows (`007-21`, `007-31`, `007-20`, `007-30`) were absorbed into HD 690 as 4 phantom rows. Fix directive: [`E03-confidence-findings/S03.3-uat-fixes.md`](E03-confidence-findings/S03.3-uat-fixes.md) § Fix 2 (option (a) confirmed: emit one section per listed HD, rows duplicated).
...
- [ ] Statewide antelope overlay extracted exactly once; emitted row distinct from per-HD antelope rows. **RE-OPENED 2026-05-08 by UAT failure:** the row IS emitted exactly once and distinct from per-HD rows, but distinct in the wrong way — interpretively synthesized rather than column-faithful. PDF column structure puts `Aug. 15-Nov. 08` in **Season Dates** (general); artifact remapped to `season_coverage.archery_only: true` based on extras-text interpretation. Extras column "First and only choice. ArchEquip only." also dropped. Fix directive: [`E03-confidence-findings/S03.3-uat-fixes.md`](E03-confidence-findings/S03.3-uat-fixes.md) § Fix 3 + Fix 4.
...
- [ ] **UAT (faithfulness, operator-owned):** **FAILED 2026-05-08.** PM ran UAT spot-check against deer HD 124, elk HD 170, antelope HD 690, antelope STATEWIDE 900-20. Six defects found across four candidates (D1: per-row window divergence collapsed to first-row's value; D2: Region 7 portions absorption into HD 690; D3+D4: STATEWIDE column-position remap + extras dropped; D5+D6: STATEWIDE prefix-strip + comma-strip inconsistency). S03.3 re-opened on `feat/S03.3-dea-booklet-extraction`. Re-runs after fix lands.
...
**Closure note (2026-05-08, partial — re-opened by UAT 2026-05-08):** Branch `feat/S03.3-dea-booklet-extraction`, 12 commits (`e1a047f..69d98f7`), cleared PR code review but **failed UAT**. Artifact at re-open: 264 sections / 1178 rows / 4 manifests committed. Code structure remains valid; UAT findings target specific extraction-logic defects rather than wholesale rewrite. Original closure recorded 9 real-PDF discoveries adapted into the implementation; one of those — "window divergence per row" — was filed as a deferred follow-up and **has now been promoted to a P0 UAT defect** (D1) because it materially affects the General Season window for ≥143 multi-row sections. Two further defects (D2: Region 7 portions absorption; D3/D4: STATEWIDE column-position remap + extras drop) were not surfaced by code review and only emerged from PDF spot-checks. See [`E03-confidence-findings/S03.3-uat-fixes.md`](E03-confidence-findings/S03.3-uat-fixes.md) for the six-defect inventory and code-level fix directives. Three new pitfalls in `.roughly/known-pitfalls.md` under Integration — pdfplumber survive from the original closure (one-table-per-page; `-` as universal absence sentinel; merged-cell sub-row carry-forward); a fourth ("Section-level first-observation-wins for per-row variable fields is interpretive, not faithful") is to be added with the UAT fix.

**Deferred follow-ups (non-blocking; surface to downstream — these survive the UAT-failure re-open):**
...
**Note:** the third deferred follow-up from the 2026-05-08 closure ("window divergence per row, first-observation-wins") was promoted to **P0 defect D1 by UAT** and is being fixed in S03.3 re-open per [`E03-confidence-findings/S03.3-uat-fixes.md`](E03-confidence-findings/S03.3-uat-fixes.md) § Fix 1. It no longer appears as a deferred follow-up.
