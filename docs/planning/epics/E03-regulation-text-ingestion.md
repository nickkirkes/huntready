...
**Status:** In Progress (4/13 stories complete; S03.3 closed 2026-05-08 after UAT-driven fix cycle)
...
**UAT Gating:** S03.3 (UAT cleared 2026-05-08 after one fix cycle), S03.4, S03.5 (per-booklet faithfulness review), S03.7, S03.10, S03.12 (entity ingestion + binding + milestone exit)
...
- **Docstring polish (small):** the UAT fix added a 4th cleanup rule — extras-only `re.sub(r"\s+", " ", normalized_extras)` collapse at `extract_dea.py:1019-1024` — that's documented inline at the write-site but not in the module-level docstring's "Cleanup rules" section (`extract_dea.py:24-48`). AC #338 reads the docstring as the canonical contract; ~5-line addition needed for strict parity. Behavior is correct and locked by tests; the gap is documentation hygiene only. Pick up when next touching the module (likely S03.4, which extends the same patterns to Black Bear).
