...
4. **`docs/runbooks/M1-uat.md` §4 criterion #6 "Expected counts" table** (handoff §8 #4) — add a footnote distinguishing **build counts** from **post-UPSERT-collapse DB counts**. Concrete deltas per handoff §8 #4 (memorialize verbatim): regulation_record 437 build → 435 DB; license_tag 1225 build → 825 DB; license_season 3040 build → 2411 DB; regulation_license 1914 build → 1279 DB; regulation_season 1385 build → 1381 DB; draw_spec 388 build → 276 DB. Entity tables collapse via `INSERT … ON CONFLICT DO UPDATE`; link tables collapse via `ON CONFLICT DO NOTHING`. Both numbers correct in their respective contexts. **Optional enhancement — implementation agent's call, flag-and-discuss with the user before adopting**: handoff §8 #4 enumerated six entity/link tables but did not include `jurisdiction_binding`. S04.3's `--dry-run` output on 2026-05-30 confirmed `jurisdiction_binding`'s build count is **788** (the same empirical value S04.2's `_BINDING_COUNT_GUARD_BAND = (552, 1024)` is centered on; matches S03.10 T16 baseline). The DB-side post-UPSERT count is also 788 (the loader UPSERTs by id and the cross-product produces no duplicates; `regulation_binding` collapses are not applicable). If the implementation agent surfaces the question, **the cleanest addition is "jurisdiction_binding 788 build → 788 DB (no collapse — id-keyed UPSERT)"** as a seventh row in the build-vs-DB table. The S04.1 + S04.2 closure work has already locked 788 as the durable reference number; adding it to the runbook table completes the build-vs-DB picture. Adopting this is a scope expansion of handoff §8 #4 — flag to the user; do not auto-expand. Spec position: handoff §8 #4 is authoritative; adoption requires user approval. PM recommendation: adopt — the build-vs-DB convention is incomplete without `jurisdiction_binding` and the number is now well-established.
