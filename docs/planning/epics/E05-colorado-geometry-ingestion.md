...
- [ ] One of three outcomes documented in story closure:
  ...
  - (c) **No-hunt zones discovered (expected per research doc `ed721c4` — 10 V1-scoped CO no-hunt zones via PAD-US 4.1: 4 NPs + 5 NMs + AFA)** → ingest with `kind='restricted_area'` and `role='no_hunt_zone'` per [ADR-021](../adrs/ADR-021-jurisdiction-binding-no-hunt-zone-role.md) (Accepted via S05.3.5 PR #58); the DDL CHECK constraint now permits the value, no ADR pause needed
