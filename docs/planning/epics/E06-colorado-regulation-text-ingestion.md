...
- [ ] `docs/planning/epics/E06-confidence-findings/S06.0.md` exists with the **5 pre-registered decisions plus the conditional 6th** (`db.update_geometry_verbatim`, the AC below) captured verbatim from the human's response (the 5 are enumerated under "Pre-registered decisions (5)" above and in the Open Questions section; the 6th is conditional on the S06.5 UPDATE-helper choice)
