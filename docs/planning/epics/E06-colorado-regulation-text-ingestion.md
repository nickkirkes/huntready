...
- [x] 2026 CPW Big Game correction PDF entered with HEAD-verified URL (`co-cpw-big-game-2026-correction-2026-02-19`; **moose-only — out of V1 scope; ADR-019 merge expected to be CO V1 no-op** — registered for provenance; S06.4 confirms inert); no `pending: true` entries — both URLs operator-resolved at S06.0/D4
...
- [x] `extracted/big-game-2026.json` deterministic SHA-256 across two consecutive extraction runs (**SHA-256 `3c2ecd90…015d`** pinned in tests); **737 sections / 2,758 rows** committed as the durable text-side fixture
...
**S06.1 forward-note (2026-06-09):** S06.1 pinned the **2026 CPW Big Game correction PDF** (`co-cpw-big-game-2026-correction-2026-02-19`, 2 pp, `document_type='correction'`) which S06.1's discovery confirmed is **moose-only** (two omitted moose hunt codes, p.61). Moose is **out of V1 species scope** per PRD 002. The ADR-019 `correction > annual_regulations` doc-type-precedence merge is therefore **expected to be a CO V1 no-op**. The correction is registered for provenance; **S06.4 must confirm inert against the full 2-page extract before declaring it so** — the canonical merge machinery from S03.4 (three-pass arbitration: Pass 1 base extraction; Pass 2 correction operations per BMU; Pass 3 merge with `applied_correction=true` + confidence demote-one-tier per ADR-017 §4) is preserved as the inert-confirmation pathway, NOT bypassed. If the 2-page extract surfaces any non-moose content for a V1 species, the S04-pattern three-pass arbitration fires per ADR-019 as it would for MT's 2026-03-18 black bear correction.
