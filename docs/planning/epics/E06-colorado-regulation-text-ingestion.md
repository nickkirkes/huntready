...
**Status:** In Progress — 9 of 15 stories closed (S06.0 / S06.1 / S06.3 / S06.4 / S06.3.1 / S06.5 / S06.6 / S06.6.1 / S06.6.2 Group A; S06.2 omitted by design at S06.3 Stage-1 discovery); **S06.6.2 Group A complete at-merge 2026-06-23** (operator-pass-discovered PAD-US 4.1 GeometryCollection area-preservation drift; **second consecutive carve-out** for the same upstream republish — S06.6.1 fixed OBJECTID/outFields, S06.6.2 relaxed `lib/arcgis.geojson_to_multipolygon_wkt` area-preservation epsilon `1e-6 → 1e-3` via Outcome (a) + closed deferred S06.6.1 metadata/manifest commits + validated all 10 V1 zones; Group B operator-pending). **S06.7 is now next active** — dispatch once the M2-build operator pass resumes from Step 4 → 10 against `main` (live execution gated on the operator Group B; S06.7 dry-run path can begin in parallel)
...
- [ ] All 10 S05.4 geometry rows (`CO-restricted-{slug}-geom`) have `verbatim_rule` populated post-load; SQL spot-check verifies non-null across all 10 — *operator-pending*
- [ ] `source` field retained as PAD-US citation per **D5 = (b) split-provenance** (NOT updated to CPW); SQL verifies `source.document_type='gis_layer'` on all 10 rows — *operator-pending*
- [ ] Per-row text faithful to source PDF; PM-run spot-check on **≥3 of the 10 zones** (e.g., Rocky Mountain NP, Mesa Verde NP, AFA) cross-checks against CPW brochure page 78 — *PM-pending after Group B operator capture*
...
- [ ] Live `supabase` row count: `SELECT COUNT(*) FROM regulation_record WHERE state='US-CO'` returns **398** post-load — *operator-pending*
- [ ] SQL-shape verification: every row has populated `source` jsonb (`document_type='annual_regulations'`); `confidence` ∈ `{high, medium}` (no `low` rows); `jurisdiction_code` matches `^CO-GMU-\d+$` for the 398 per-GMU records (0 CO-STATEWIDE rows in V1) — *operator-pending*
- [ ] Per-species count verification: mule_deer 141 / elk 115 / pronghorn 77 / whitetail 19 / bear 46 (sum 398) — *operator-pending*
...
**🚩 S06.5 forward-note (2026-06-18; user-decided 2026-06-17) — AFA classification correction is REQUIRED at S06.10 entry**: S06.5 closure surfaced that the Air Force Academy row (`CO-restricted-united-states-air-force-academy-geom`, one of the 10 S05.4 `kind='restricted_area'` rows) is **a regulated-access HUNTING area, NOT a no-hunt zone** (GMU 512 carries CPW hunt codes; CPW publishes escorted rifle deer hunts there per CPW Big Game brochure p.78). S06.5's `verbatim_rule` for that row now describes *how to hunt there*, not a closure. **S06.10 must NOT bind AFA `role='no_hunt_zone'`** — doing so would protect a huntable zone as closed in query results, contradicting CPW-published reality and the user's stated intent ("AFA must remain protected as a huntable zone in query results"). **Two resolution paths** for the S06.10 implementer to choose between (or surface for the human if both feel wrong):

- **(a) Bind AFA `role='other_overlay'`** at the binding-loader layer — keeps the geometry row as-is (`kind='restricted_area'`; PAD-US source), but the binding semantics correctly express "restricted access, not closed." Mirrors MT's pre-ADR-021 `other_overlay` disposition. Simplest fix.
- **(b) Revisit the S05.4 federal no-hunt-zone classification at the geometry layer** — change the AFA row's `kind` away from `restricted_area` (e.g., to a new `kind='hunting_district'` analog or a new value), accept the schema implications, and bind via the appropriate per-state convention. Higher-cost; requires geometry-side migration + ADR-018-style three-place sync.

**PM recommendation: option (a)** for V1 — it preserves the geometry row, requires no schema migration, and gives S06.10 the semantic precision needed. Option (b) is reserved for V2 if hunting-permission-bearing federal lands become numerous. **The other 9 S05.4 rows (4 NPs + 5 NMs) are genuine no-hunt zones** — `role='no_hunt_zone'` is correct for them (the S06.5 phrasing-case (1) NPS sentence is the verbatim closure rule). S06.10 implementer must surface this decision at story open (the binding-loader's no-hunt-zone code path must split: 9 rows → `no_hunt_zone`, 1 row → `other_overlay`). See Known Issue #12 below.
