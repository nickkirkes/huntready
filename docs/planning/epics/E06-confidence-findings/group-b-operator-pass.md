...
3. **`verbatim_rule` is 1238 chars, not the 1288 stated in the S06.9.1 closure narrative + CLAUDE.md.** The substantive S06.9.1 full-prose fix is unquestionably present (interior `five working days` anchor confirmed; far beyond the old ~50-char heading-only text), so this reads as a **doc transcription error (1238↔1288 digit-swap)** — same number-drift class as the project's documented 172/173-records and 9/10-zones corrections. **Not a wrong-artifact problem; not blocking.** **Corrected at source in this pass:** `1288 → 1238` fixed in CLAUDE.md, docs/planning/README.md, the E06 epic, the S06.9.1 closure note, and the S06.9.1 plan. PM still runs the byte-for-byte UAT against brochure p.73 using the captured text above.
4. **S06.7 row total in CLAUDE.md / docs/planning/README.md was wrong: 8,979 should be 10,979.** The five live-verified counts (2013 + 2470 + 2013 + 2013 + 2470) sum to **10,979**, not the "8,979 entity/link rows" stated in the S06.7 closure narrative (off by exactly 2,000 — a rolled-up-total arithmetic error; the five constituent counts are correct everywhere). The live post-write snapshot sums to 10,979, confirming. **Not a data problem** — the loader wrote the correct rows. **Corrected at source in this pass:** `8,979 → 10,979` fixed in CLAUDE.md (1×) and docs/planning/README.md (6×); the E06 epic does not carry this rolled-up figure. This working note also inherited the wrong figure and was corrected.

## Counts for PM rollup

- **S06.7:** season_definition 2013 / license_tag 2470 / license_season 2013 / regulation_season 2013 / regulation_license 2470  (10,979 rows)
...
- [x] (Doc hygiene) Corrected the S06.7 row total `8,979 → 10,979` (CLAUDE.md + docs/planning/README.md) and the `verbatim_rule` length `1288 → 1238` (CLAUDE.md, README, E06 epic, S06.9.1 note + plan) at source this pass — see Anomalies #3/#4.
