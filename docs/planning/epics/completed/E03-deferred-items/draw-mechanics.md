...
at run time naming the conflict + rationale. The override is applied
symmetrically to both `draw_spec.quota` and `license_tag.quota` on all 4
affected rows, so the two tables agree on the canonical drawable value (300);
the cross-listed quota=200 values are dropped from both columns. Per-HD-cap=200
values live only in this deferral entry's rationale +
`_KNOWN_CROSS_LISTING_OVERRIDES` in `load_draw_specs.py`. The home-HD
verbatim_rule on each per-HD `license_tag` preserves the "Valid on private
lands in HDs 211, 212, 216 and south portion of 210" language.
