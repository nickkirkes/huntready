...
- [x] `ingestion/states/montana/load_jurisdiction_bindings.py` exists (~870 LOC, state-agnostic-clean per `TestNoLibImports` AST guard); reads overlay fixture + queries regulation_record table; deterministic load order
- [x] **Per-regulation_record traversal:** each regulation_record produces its own set of binding rows via overlay-fanout; no shortcut "for every overlay row, find a regulation"
- [x] **Cross-species filter:** `is_binding_eligible(regulation_record, overlay_row)` implements the per-species table; covers all four discriminators (per-species row, `MT-restricted-elk-` elk-only, `MT-HD-whitetail-` whitetail-only, statewide-pronghorn → `MT-STATEWIDE-geom`). Fail-loud on unknown `species_group` (caught both in the public function AND independently inside the builder).
- [x] Statewide regulation_records bind to `MT-STATEWIDE-geom` with `role='primary_unit'`; **pair-aware** missing/unexpected-statewide guards on `(jurisdiction_code, species_group)` so a wrong-species statewide anchor or a new statewide anchor added without ADR-018 amendment fails loud.
- [x] No-hunt-zone Option A applied via Spec Deviation #4: single-clause `extensions.ST_DWithin(zone.geom, hd.geom, 5000)` on native geography type (boundary-to-boundary). Spec's two-clause `ST_Touches OR ST_DWithin(centroid, centroid, 5000)` returned 0 matches for ALL 3 orphans at T0 because park centroids are >5km from every HD centroid (Yellowstone ~30km deep). SQL is `extensions.`-prefixed (Supabase PostGIS lives in `extensions` schema), filters by `hd.state = 'US-MT'`, filters by `license_year`, and excludes `kind='portion'` (post-merge fix for silent skips).
- [x] **Per-zone zero-binding fail-loud:** loader exits non-zero with the zone's id surfaced if any of the 3 `EXPECTED_RA_ORPHAN_IDS` produces zero binding rows; locked by `TestQueryNearbyHdsForZone` synthetic-100km-zone test.
- [x] Row-count guard `_BINDING_COUNT_GUARD_BAND = (552, 1024)` fires BEFORE `db.connect()` per OQ7 discipline; ±30% around T16's empirical 788 measured 2026-05-28 (narrowed in E04 S04.2 from the prior `(400, 1100)` T16-pending band)[^row-count-correction]
...
- [x] `source` jsonb on every binding row references the **original ArcGIS layer** that produced the child geometry via adapter-local SELECT against `geometry.source` (Spec Deviation #3 — no new `db.py` helper since `db.py` is reserved for write/mutation per S03.10 constraint #7); `Pydantic ValidationError` wrapped with the failing geometry id (review-fix from triad CRITICAL C2).
- [x] `geometry-overlays-dropped.json` is NOT consumed (per ADR-016)
- [x] `confidence` not written (per ADR-017 spatial-confidence carve-out — no column on the entity)
- [x] **UAT:** spot-check queries in working note T15 (HD 262 elk binding count + role distribution; cross-species spot-checks; no-hunt-zone fan-out; bear-binding UPSERT no-op stability). T16 live UAT is **operator-pending** — regulation_record table empty at T0 probe (2026-05-23); requires batch run of S03.0/S03.6/S03.6.1/S03.7/S03.8/S03.9 before T16. **Does not gate S03.11 or S03.12 planning.**
- [x] UPSERT idempotency: re-deriving the S03.6.1 bear binding produces a byte-identical id (locked by 3 separate tests: `TestBuildStatewideBindings::test_bear_binding_id_byte_identical_to_s036_1_locked_value`, `TestIdStability::test_statewide_bear_id_locked`, `TestRealArtifactRegression::test_bear_statewide_id_is_upsert_noop_with_s036_1`). UPDATE clause excludes identity fields per S03.6.1's `db.upsert_jurisdiction_binding` contract.
- [x] **Working note** at `docs/planning/epics/E03-confidence-findings/S03.10.md` records T0 probe results, fan-out math, post-review fix logs, M2 follow-ups, operator handoff for T16. Deletes at m1 tag per ADR-017 §6.
- [x] `ruff check` clean; `mypy ingestion/lib/ states/montana/load_jurisdiction_bindings.py` clean per-file (PM-verified per S03.7 discipline-reminder).
- [x] Unit tests in `ingestion/tests/test_load_jurisdiction_bindings.py` (~1530 LOC) cover regulation_record-to-geometry matching (incl. statewide), per-species cross-species-filter behavior, no-hunt-zone nearby-rule logic, source-citation construction (cites source layer not overlay fixture), and all **18 fail-loud guards** added across review-triad + 4 PR-review iteration rounds. **Suite reaches 1128 passed + 2 skipped** (+104 net from S03.10).

**Closure note (code-complete 2026-05-26; PR `e83ef2d` squash-merged to main from `feat/S03.10-jurisdiction-binding-generation`):** 8 commits past S03.6.1 close (main impl `3003d46` + 7 review-driven fixes through `3a851b5`). First **cross-cutting** binding loader; reads `geometry-overlays.json` × regulation_record cross product, applies per-species filtering, walks 4 binding sources (statewide / overlay self-row / overlay portions / no-hunt-zone Option A), UPSERTs in one atomic three-phase transaction via `db.upsert_jurisdiction_binding` reused from S03.6.1 (no `db.py` changes). **T16 live UAT operator-pending** — regulation_record table empty at T0 probe (2026-05-23); requires batch run of S03.0/S03.6/S03.6.1/S03.7/S03.8/S03.9 before binding writes can occur. Operator runbook in working note. Projected ~1000 bindings (2 statewide + ~940 overlay + ~59 no-hunt-zone) inside `[400, 1100]` guard band; narrow to ±30% around observed count after T16.

**Four PM-approved spec deviations** (footnoted in epic at L1045, L1070, L1092, L1311/L1313):

1. **ID format** — imports `_JURISDICTION_BINDING_ID_FORMAT` from `load_regulation_records.py` (S03.6.1's locked format). Spec L1045's `binding-{geometry_id}-{role}` was stale; importing the constant preserves the S03.6.1 bear-binding UPSERT no-op contract.
2. **Count band** — `[400, 1100]` intentionally wide pending T16; spec's 1,500-3,500 was ~2x too high. Narrow after T16.
3. **Source attribution** — adapter-local SELECT against `geometry.source` rather than a new `db.py` helper (per constraint that `db.py` is reserved for write/mutation helpers).
4. **No-hunt-zone "nearby" rule** — single-clause `extensions.ST_DWithin(zone.geom, hd.geom, 5000)` on native geography (boundary-to-boundary, canonical PostGIS pattern) replaces the spec's two-clause `ST_Touches OR ST_DWithin(centroid, centroid, 5000)` which returned 0 matches for all 3 orphans at T0. No new ADR required — refines implementation of an existing rule.

**18 fail-loud guards** added across the review-triad + 4 PR-review iteration rounds (all locked by tests): overlay-row-not-dict; overlay-row-missing-keys (names index + present keys); unknown `role_for_e03` (caught twice — `_VALID_ROLE_FOR_E03` gate + independent `is_binding_eligible` check); unknown `species_group`; unhandled `jurisdiction_code` pattern in `_derive_parent_geometry_id`; per-builder duplicate-id (overlay + no-hunt-zone); cross-builder global duplicate-id (review-fix C1); per-zone zero-nearby-HDs (AC #1091); empty regulation_record table with diagnostic naming prior loaders (review-fix W1); pair-aware missing-statewide on `(jurisdiction_code, species_group)` (review-fix-to-review-fix catching wrong-species corruption); symmetric pair-aware unexpected-statewide (catches new anchors without ADR-018 amendment); reg_record's `parent_geometry_id` has no overlay entries (review-fix P1); missing `source_lookup` with diagnostic; geometry source jsonb malformed (names failing geometry id; review-fix C2); row-count band `[400, 1100]`; SQL filters HDs by state (prevents M2 Colorado cross-state silent under-binding; review-fix P2); SQL filters by `license_year` (prevents multi-year fan-out; review-fix P2); SQL excludes `kind='portion'` (post-merge fix for silent skips).

**Seven new pitfalls** in `.roughly/known-pitfalls.md` (3 under Integration — Supabase / PostGIS; 4 under Conventions — Ingestion adapters): (A) `geometry.geom` column not `geog` despite CLAUDE.md prose; (B) PostGIS in `extensions` schema in Supabase — all `ST_*` calls must be `extensions.`-prefixed in raw SQL (E02 + S03.10 both hit this); (C) centroid-to-centroid `ST_DWithin` is zero-matches for large polygons — use boundary-to-boundary geography `ST_DWithin(geom, geom, distance)`; (D) `with db.connect()` commits-on-exit + rolls-back-on-exception — explicit `conn.commit()` is the real gate, exit-commit is a safety net; (E) per-builder dedup sets miss cross-builder collisions — multi-builder adapters need global dedup in `main()` before write; (F) `Model.model_validate(...)` inside dict-comprehensions swallows row identity — use explicit for-loop + try/except naming the id; (G) **"Live-DB write is operator-driven"** — closure-note row counts describe dry-run-verified shape; live writes happen only when operator invokes the loader. Future stories depending on prior loaders' DB state must run them first or mock.

**Q19 NOT propagated to this module** — `db.upsert_jurisdiction_binding` already has a stronger mitigation than the 3 Q19-affected helpers: UPDATE clause excludes identity fields entirely (S03.6.1 OQ-S6.1-4), so identity drift on UPSERT is impossible at the SQL layer. S03.10 does not introduce a local module-load drift guard. **Q19 ADR remains a pre-M2 blocker** for the 3 helpers it does affect (`_UPSERT_SEASON_DEFINITION_SQL`, `_UPSERT_LICENSE_TAG_SQL`, `_UPSERT_REPORTING_OBLIGATION_SQL`).

**Five M2-deferred items recorded:** (1) `role='other_overlay'` semantic awkwardness for no-hunt-zone bindings — DDL enum has 7 values; none semantically captures "this HD's regulations don't apply inside this zone"; V1 ships with `other_overlay` (only DDL-permitted choice); M2 candidate: add `'no_hunt_zone'` role enum (requires DDL migration + ADR). (2) Future portion-keyed reg_records — 3 coordinated touchpoints if any future state introduces them: extend `_derive_parent_geometry_id`; re-introduce portions to `_query_nearby_hds_for_zone` SQL; update `TestQueryNearbyHdsForZone::test_sql_excludes_portions_regression_guard`. (3) Cross-state spatial pollution in M2+ — Colorado adapter must filter by `_STATE = 'US-CO'`; pattern is established. (4) Count band narrowing — after T16 produces empirical count, narrow `_BINDING_COUNT_GUARD_BAND` to ±30% around observed value; update AC #1087 footnote. (5) Cell-level source attribution — V1 attributes binding source from single-geometry-source; multi-source provenance needs schema field if a future state has multi-source HDs.

**No ADRs created** — implementation refines ADR-010 (decomposed entities — first cross-cutting loader), ADR-014 (source-citation `gis_layer` document type), ADR-016 (overlay fixture provenance), ADR-017 (spatial-confidence carve-out), ADR-018 (statewide bindings + `MT-STATEWIDE-geom`).

**S03.10 produces no audit signal for S03.11's confidence-calibration framework** — per ADR-017 §2, `jurisdiction_binding` has no `confidence` column. S03.11's audit subjects are the entities from S03.6 / S03.7 / S03.8 (regulation_record, season_definition, license_tag, draw_spec). **S03.11 is now unblocked.**

**S03.11 was queued next.**
...
- [x] Synthesis report `docs/planning/epics/E03-confidence-calibration-synthesis.md` (~360 lines, PM-owned, survives m1 tag) consolidates all per-story working notes. §1 edge-case catalogue (10 cases); §2 sample (50 rows); §3 audit results + scoped pass-rate table (regulation_record.confidence 39/39, EC8 parent-inheritance 6/6, EC9 license_tag row-level 5/5 = combined 50/50 = 100%); §3.1 tier distribution; §4 open issues; §5/§7 trigger analysis with PM-deliberation/user-decision distinction explicit; §6 verdict (FINALIZE).
- [x] Stratified audit performed: 50 rows across 10 documented edge cases (EC1-EC10). Per-EC: 5 each for EC1/EC2/EC5/EC6/EC7/EC9/EC10; EC3 = 7 (5 stratified + 2 random fill); EC4 = 2 (full population — only 2 V1 statewide overlays); EC8 = 6 (3 `sex_threshold` + 3 `quota_threshold` subtypes). Artifact-based methodology per S03.10's T0 probe finding `regulation_record` empty 2026-05-23; sources `dea-2026.json` + `black-bear-2026.json`. **Strict 4-step anti-circularity discipline enforced** (pull F12 inputs → apply ADR-017 rules independently → reveal artifact value → compare). All 50 rows match the framework.
- [x] **Confidence-assignment inputs (F12) persisted** via retrofits to 4 prior working notes: `S03.3.md` (5 row-level DEA examples + count narrative aligned to committed artifact: 272 sections / 1190 rows / 601 HIGH + 589 MEDIUM); `S03.4.md` (3 BMU examples: 100, 411, 700); `S03.6.md` (load-bearing 10-record F12 trace covering deer fan-out, MIN-aggregation, statewide overlay, bear correction-touched, Region 7 portions; HD-XXX → HD 250 backfilled; HD-YYY documented as intentional null-result); `S03.6.1.md` (MT-STATEWIDE-bear/bear with `correction_touched=False`).
- [x] **No defer-trigger fires (under user-selected INTENT reading):** Trigger 1 — all 10 edge cases map deterministically to single tiers; Trigger 3 — pass-rate 100% ≥ 80%; Trigger 2 — `low=0` is absence-by-data-property, not absence-by-framework-gap (LOW rule exists in `_assign_row_confidence`, is unit-tested by `TestDemoteOneTier`, behaves correctly when input conditions present; V1 MT data is well-formed enough not to fall through to LOW). **ADR-017 stands as-is** (active file unmodified — verified `git diff --name-only -- docs/adrs/ADR-017-confidence-calibration.md` empty across all S03.11 commits; status remains `Accepted`; all 7 numbered sections unchanged). **Q11 marked RESOLVED 2026-05-27** in `docs/open-questions.md`.
- [x] **Defer path was exercised (PM-only) and rejected at user-review time.** PM's original recommendation 2026-05-26 was PARTIAL DEFER under LITERAL reading of Trigger 2; independent architectural review by `roughly:epic-reviewer` returned LAND-WITH-EDITS on the DRAFT; PM applied 4 review-recommended refinements. User decision 2026-05-27 overrode to FINALIZE under INTENT reading. ADR-017 amendment DRAFT was created, reviewed independently, refined, then rejected at user-review time and deleted as part of the FINALIZE workflow. PM's full deliberation preserved in synthesis report §5.2 + §6 as audit-trail (not erased by override). **PM does NOT modify active ADRs autonomously — DRAFT-file pattern is the canonical handoff mechanism.**
- [x] **S03.12 does NOT block on ADR-017 amendment review** — moot in the FINALIZE outcome; no amendment-pending state carries forward to M1→M2 handoff. Auto-defer fallback unused.
- [x] **Working notes deletion queued for S03.12's final commit** (`git rm -r docs/planning/epics/E03-confidence-findings/` per ADR-017 §6); `.gitignore` update to prevent re-creation. Synthesis report at `docs/planning/epics/E03-confidence-calibration-synthesis.md` lives outside the deletion target and survives.

**Closure note (closed 2026-05-27; PR #42 squash-merged as `b84955c`, on main at merge commit `7021bda` from `feat/S03.11-confidence-calibration`):** Verdict **FINALIZE**. **ADR-017 active file unmodified**; Q11 RESOLVED 2026-05-27. Audit shape: 50-row stratified sample × 10 edge cases, 50/50 = 100% pass-rate scoped per ADR-017 §3 (regulation_record.confidence 39/39 + EC8 parent-inheritance 6/6 + EC9 license_tag row-level 5/5). Tier distribution across projected 437 `regulation_record` rows: `high=32 (7.3%), medium=405 (92.7%), low=0 (0.0%)`. **Defer-or-finalize arc**: PM recommended PARTIAL DEFER under LITERAL reading of Trigger 2 (`low=0` literally fires); independent `roughly:epic-reviewer` returned LAND-WITH-EDITS on PM's DRAFT; PM applied 4 refinements; user overrode to FINALIZE under INTENT reading (`low=0` is absence-by-data-property, not absence-by-framework-gap); DRAFT rejected and deleted. PM deliberation preserved in synthesis §5.2 + §6. **8 stale-reference fixes** as a separate sweep within the same PR (root cause: prior `docs/plans/` → `.roughly/plans/` migration via `git mv` preserves history but doesn't grep for path references): `docs/open-questions.md` Q15 Resolution-home link; 4 working notes; S03.6 plan self-reference; `ingestion/states/montana/load_seasons_and_licenses.py` module docstring; `ingestion/tests/test_extract_black_bear.py` skip-reason string. **One new pitfall** in `.roughly/known-pitfalls.md` under Conventions — Documentation & planning discipline: "Deferred open-question verdicts require an amendment-pending breadcrumb in `docs/open-questions.md`" (generalizable lesson; applies to any future audit that DEFERS rather than RESOLVES, even though this audit went FINALIZE). **Four generalizable lessons captured**: (1) deferred audits need an open-question breadcrumb that survives m1 working-note deletion; (2) anti-circularity discipline for framework audits — derive `framework_predicted` from F12 inputs INDEPENDENTLY of artifact's pre-computed value (artifact is what's being tested, not the source of truth for the test); (3) scoped pass-rate calculations — when sample rows audit different schema layers, headline pass-rate must be scoped to the canonical entity per ADR-017 §3, not rolled together; (4) PM does NOT modify the active ADR autonomously — DRAFT-file pattern is the canonical handoff mechanism even when the user overrides. **No new ADRs**; no test changes (suite holds at 1128 + 2 skipped — doc-only PR). **S03.12 is the final M1 story** and inherits two PM-closure items the S03.11 plan deferred from this story to Stage 8: CLAUDE.md L11 + E03 epic closure narrative for S03.11 (both folded into this same PM-closure commit per standard workflow — not deferred to S03.12).

**Locked context for S03.12 (M1 UAT + handoff):**

1. **M1 UAT criterion #8** ("ADR exists documenting Q11 confidence calibration") is **satisfied as-is** by ADR-017 unmodified.
2. **No amendment-pending state** carries into M1→M2 handoff document.
3. **Working-notes deletion** (`git rm -r docs/planning/epics/E03-confidence-findings/`) + `.gitignore` update happen in S03.12's final commit. Synthesis report survives.
4. **S03.10 T16 live UAT remains operator-pending** — independent of S03.12; does NOT gate m1 tag per S03.10 closure note. S03.12 may run the operator batch as part of its own live-DB UAT against the 7 PRD success-criteria queries.
...
| **N4** | Naming — `jurisdiction_binding.id` format | `f"{state}-{jurisdiction_code}-{species_group}-{license_year}-{role}-{geometry_id}"` (per `[^id-format-correction]` footnote — the originally-spec'd `...-binding-{geometry_id}-{role}` form was replaced by S03.6.1's as-shipped format; no `binding-` literal, role-then-geometry_id ordering). Locked in code as `_JURISDICTION_BINDING_ID_FORMAT` in `ingestion/states/montana/load_regulation_records.py`; imported by S03.10 (no local redefinition). Opaque-but-deterministic; downstream code MUST NOT parse the id. |
