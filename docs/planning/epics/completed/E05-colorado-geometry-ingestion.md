...
**Audited:** 2026-06-05 — see [E05-audit.md](completed/E05-audit.md). All 9 stories (S05.0–S05.7 + S05.3.5). ~80 Group A ACs MET, ~43 Group B ACs PARTIAL (operator-driven live verification, by design — identical posture to E02 S02.6/S02.7 UAT), 0 NOT MET. No blocking findings. E05 ships clean. **🟢 Group B closed end-to-end on dev via M2-build operator pass 2026-06-23** (PR #74 / `a6cec6b`; env=dev / M2-build per the runbook's Pass-mode section; future M2-release pass against prod writes alongside rather than overwriting). All 6 stories with operator-pending Group B ACs (S05.0 / S05.2 / S05.3.5 / S05.4 / S05.5 / S05.7) now show ticked + annotated checkboxes inline; verbatim outputs live in `docs/runbooks/M2-operator-pass.md` capture form.
...
- [x] **Post-write verification queries return expected results** — *closed via M2-build operator pass 2026-06-23 (PR #74 / `a6cec6b`; env=dev / M2-build)*: AC #7 satisfied — `CO-STATEWIDE-geom` written; identity columns + `document_type='gis_layer'` confirmed; `ST_IsValid` true; `ST_NumGeometries` = 1; `area_km2` populated. Verbatim outputs in `docs/runbooks/M2-operator-pass.md` capture form (Step 2); future M2-release pass will append its own verification record alongside this dev capture
- [x] **`area_km2` within 1% of Colorado's published area (~269,837 km²)** — *closed via M2-build operator pass 2026-06-23 (env=dev / M2-build)*: AC #8 satisfied — area 269,605 km² (in ±1% band; ~0.086% under CPW published 269,837 km²; well within tolerance for TIGER state-line digitization precision)
...
- [x] All rows pass `ST_IsValid` post-insert (using the Supabase `ST_GeomFromText(ST_AsText(geom), 4326)` round-trip cast workaround from `.roughly/known-pitfalls.md`) — *closed via M2-build operator pass 2026-06-23 (PR #74 / `a6cec6b`; env=dev / M2-build)*: 186 GMUs written; 0 invalid post-`ST_IsValid` check
- [x] UPSERT semantics confirmed: re-running the load produces identical state (same row count, no duplicates, no `license_year` drift); analog of S02.2's idempotency AC — *closed via M2-build operator pass 2026-06-23 (env=dev / M2-build)*: Step 3 idempotency verified; CPW FeatureServer `returnCountOnly` cross-check matches 186
- [x] Layer metadata fixture committed at `ingestion/states/colorado/fixtures/CPWAdminData-6-metadata-<timestamp>.json` (~7KB) — *closed via M2-build operator pass 2026-06-23 (env=dev / M2-build)*: live fetch metadata fixture committed via PR #74
- [x] Per-fetch manifest committed at `ingestion/states/colorado/fixtures/CPWAdminData-6-manifest-<timestamp>.json` (~5KB) with `features_count`, `layer_hash`, `hash_distribution`, `fetched_at`, `source_url`, `source_layer_max_record_count`, `source_layer_object_id_field` per S02.7's spec — *closed via M2-build operator pass 2026-06-23 (env=dev / M2-build)*: live manifest committed via PR #74
...
- [x] `supabase db push` applies the migration cleanly to the production project — *closed via M2-build operator pass 2026-06-23 (PR #74 / `a6cec6b`; env=dev / M2-build; future M2-release pass repeats against prod)*: migration `20260603000000` applied cleanly to dev Supabase project `eklivzoomtdluedzlyai`
- [x] `information_schema.check_constraints` shows the updated `jurisdiction_binding_role_check` constraint with 8 values — *closed via M2-build operator pass 2026-06-23 (env=dev / M2-build)*: enum 7 → 8 values confirmed; exact `check_clause` captured in `docs/runbooks/M2-operator-pass.md` capture form (Step 1)
- [x] `SELECT DISTINCT role FROM jurisdiction_binding` returns a subset of {`primary_unit`, `portion`, `restricted_area`, `cwd_management_zone`, `bear_management_unit`, `block_management_area`, `other_overlay`, `no_hunt_zone`} — *closed via M2-build operator pass 2026-06-23 (env=dev / M2-build)*: distinct roles confirmed to be within the 8-value enum
- [x] The 3 MT rows now have `role = 'no_hunt_zone'` (`SELECT id, role FROM jurisdiction_binding WHERE geometry_id IN ('MT-restricted-bigame-glacier-national-park-geom', 'MT-restricted-bigame-sun-river-game-preserve-geom', 'MT-restricted-bigame-yellowstone-national-park-geom')`) — *closed via M2-build operator pass 2026-06-23 (env=dev / M2-build)*: **all 50 MT fan-out bindings across the 3 geometry IDs** (operator-pass-confirmed clarification of the runbook's "3 rows" prose; the UPDATE keys on `geometry_id` so all bindings per zone reclassify; geometry_id-count × ~17 bindings/zone = ~50) reclassified `other_overlay → no_hunt_zone`
- [x] Service-role row count on `jurisdiction_binding` is unchanged pre/post migration (DDL-only + targeted UPDATE; touches exactly 3 rows; no INSERT or DELETE) — *closed via M2-build operator pass 2026-06-23 (env=dev / M2-build)*: total binding row count 788 unchanged; UPDATE touched all 50 fan-out bindings across 3 geometry IDs (not "3 rows" — see prior AC clarification)
...
- [x] Live PAD-US fetch against the production service-role DSN: 11 features returned, Curecanti dropped post-fetch, 10 rows UPSERTed to `geometry` — *closed via M2-build operator pass 2026-06-23 (PR #74 / `a6cec6b`; env=dev / M2-build; gated on S06.6.1 + S06.6.2 fixes for the two PAD-US 4.1 republish drifts)*: 11 features fetched; Curecanti dropped per 36 CFR §2.2; 10 rows UPSERTed
- [x] `returnCountOnly=true` cross-check against the PAD-US source confirms 11 raw features in the V1 WHERE clause — *closed via M2-build operator pass 2026-06-23 (env=dev / M2-build)*: cross-check matches
- [x] Post-insert `ST_IsValid` round-trip verification on all 10 rows — *closed via M2-build operator pass 2026-06-23 (env=dev / M2-build)*: all 10 valid post-`ST_IsValid` round-trip
- [x] `GIS_Acres` ±10% acreage sanity check against an independent reference — *closed via M2-build operator pass 2026-06-23 (env=dev / M2-build)*: computed acres match PAD-US `GIS_Acres` to within tolerance; RMNP specifically matches `GIS_Acres` to 0.04% (the 0.0676% gap between `parsed.area` and `recovered.area` is the self-intersection artifact removed by `make_valid()`, per S06.6.2 Phase A characterization)
- [x] Live-run metadata fixture + manifest committed to `ingestion/states/colorado/fixtures/` (mirroring S05.0 / S05.2 Group B fixture-commit pattern) — *closed via M2-build operator pass 2026-06-23 (env=dev / M2-build)*: PAD-US Group B fixture committed via S06.6.2 Phase D as `Federal_Fee_Managers_Authoritative_PADUS-0-{metadata,manifest}-20260623T000955Z.json` per the metadata+manifest convention (features payload stays gitignored)
...
- [x] `ingestion/states/colorado/fixtures/geometry-overlays.json` generated and committed — (`gmu`, `gmu`) self-references for every GMU row + GMU↔CWD-zone (vacuously empty per S05.3 gap) + GMU↔Restricted-Area covers/intersects per ADR-016 thresholds — *closed via M2-build operator pass 2026-06-23 (PR #74 / `a6cec6b`; env=dev / M2-build)*: 201 rows (186 gmu-self + 15 gmu→RA intersects); CWD-zone joins vacuously empty per S05.3 documented gap
- [x] **Paired audit log** at `ingestion/states/colorado/fixtures/geometry-overlays-dropped.json` committed alongside kept fixture; sorted, deterministic — *closed via M2-build operator pass 2026-06-23 (env=dev / M2-build)*: audit log committed alongside kept fixture
- [x] **Threshold recalibration check**: closure note documents the audit-log inspection (borderline drops in `[0.005, 0.02]`; borderline relabels in `[0.98, 0.995]`); MT thresholds preserved unless either band differs from MT proportions by >10% (recalibrate per ADR-016 §4 if outside) — *closed via M2-build operator pass 2026-06-23 (env=dev / M2-build)*: MT thresholds preserved; coverage invariant held
- [x] **UAT — visual spot-check**: closure note documents inspection of multi-part GMU self-rows (consuming `multipart-gmus.json` from S05.2) + no-hunt-zone orphans appearing in `EXPECTED_CO_RA_ORPHAN_IDS` — *closed via M2-build operator pass 2026-06-23 (env=dev / M2-build)*: multi-part GMU 20 used as spatial-test-points anchor (S05.7); 10 RA orphans match `EXPECTED_CO_RA_ORPHAN_IDS`
- [x] Every fixture-referenced `geometry_id` exists in the CO geometry list (JSON-level FK check) — *closed via M2-build operator pass 2026-06-23 (env=dev / M2-build)*: 0 dangling FK confirmed
- [x] Both fixture files are reproducible — byte-identical JSON across two consecutive runs (sorted, `sort_keys=True`, `indent=2`, trailing newline, atomic tmp+rename, `overlap_pct` rounded to 6 decimals per S02.6) — *closed via M2-build operator pass 2026-06-23 (env=dev / M2-build)*: reproducible SHA confirmed across two consecutive runs
...
**Group A deliverables landed at-merge:** runbook `docs/runbooks/E05-colorado-geometry-verification.md` (363 LOC; 7 verification sections + CO-specific Section 0 fixture-generation + Section 5 `ST_Envelope` CO-bounds check; every `ST_*` call `extensions.`-prefixed; every query carries `AND state = 'US-CO'`; reproducibility section sequences `DELETE FROM jurisdiction_binding WHERE geometry_id IN (SELECT id FROM geometry WHERE state = 'US-CO')` before `DELETE FROM geometry WHERE state = 'US-CO'` with explicit `TRUNCATE` guard); working note `docs/planning/epics/E05-confidence-findings/S05.7.md`; post-implementation audit `docs/planning/epics/completed/E05-audit.md`; this epic's Status + `Audited:` + AC updates.
...
6. **Reproducibility (topological)**: Wipe + re-ingest pattern. **CRITICAL**: per the E02-cascade callout (E02 runbook line 189), `jurisdiction_binding` now FK-references `geometry.id`. The CO wipe MUST sequence: `DELETE FROM jurisdiction_binding WHERE geometry_id IN (SELECT id FROM geometry WHERE state = 'US-CO')` BEFORE `DELETE FROM geometry WHERE state = 'US-CO'`. The runbook MUST include an explicit guard against `TRUNCATE` (would wipe MT). Mirrors S02.7's reproducibility protocol with the post-E04 FK cascade adapted.
...
- [x] `ingestion/states/colorado/fixtures/spatial-test-points.json` exists with ≥1 named test point per `kind` value present in CO geometry, including: ≥3 GMUs (with ≥1 multi-part anchor from S05.2's `multipart-gmus.json`); a CWD-zone test point only if S05.3 produced rows (it did not — CO publishes no CWD zones per the 2026-06-03 documented gap, so this point is omitted); ≥1 restricted area if S05.4 produced any; ≥1 statewide point; ≥1 outside-CO negative-control point — *closed via M2-build operator pass 2026-06-23 (PR #74 / `a6cec6b`; env=dev / M2-build)*: 7 points generated incl. multi-part anchor GMU 20
- [x] Every coordinate is a real `shapely.representative_point()` from the actual loaded geometry (no invented points) — *closed via M2-build operator pass 2026-06-23 (env=dev / M2-build)*: generator used live CO geometry; no invented points
- [x] **UAT — `ST_Covers` spot-check**: Each fixture point resolves correctly via `extensions.ST_Covers(geom, extensions.ST_GeogFromText(...))` per the runbook's section 1 verification protocol; **every spot-check SQL block includes `AND state = 'US-CO'`** in the WHERE clause per PRD 002 success criterion #4 — *closed via M2-build operator pass 2026-06-23 (env=dev / M2-build)*: 7-section spatial UAT passed; WY negative-control returned 0 rows confirming PRD 002 SC #4
- [x] All CO geometry rows pass `extensions.ST_IsValid` (Supabase round-trip cast workaround) — *closed via M2-build operator pass 2026-06-23 (env=dev / M2-build)*: 197 CO rows valid (1 statewide + 186 GMU + 10 RA)
- [x] **Named multi-part anchor verification**: first entry in `multipart-gmus.json` returns `ST_NumGeometries > 1` via the runbook's section 3 protocol; if `multipart-gmus.json` is empty (CO unexpectedly has zero multi-part GMUs), document the verification as N/A in the closure note and surface as a data observation — do NOT silently skip the AC — *closed via M2-build operator pass 2026-06-23 (env=dev / M2-build)*: multi-part anchor GMU 20 confirmed; `ST_NumGeometries > 1` verified
- [x] **CO-bounds post-load `ST_Envelope` check**: `extensions.ST_Envelope(extensions.ST_Collect(extensions.ST_GeomFromText(extensions.ST_AsText(geom), 4326)))` over `state='US-CO'` returns a bbox within `[-109.06, -102.04] × [36.99, 41.00]` — *closed via M2-build operator pass 2026-06-23 (env=dev / M2-build)*: bbox within CO bounds
- [x] Geography GiST index reachability documented per S02.7 protocol section 4; `EXPLAIN ANALYZE` plan captured in runbook — *closed via M2-build operator pass 2026-06-23 (env=dev / M2-build)*: GiST index used per `EXPLAIN ANALYZE` plan captured in runbook capture form
- [x] **Reproducibility section in the runbook** correctly sequences `DELETE FROM jurisdiction_binding WHERE geometry_id IN (SELECT id FROM geometry WHERE state = 'US-CO')` BEFORE `DELETE FROM geometry WHERE state = 'US-CO'`; includes explicit guard against `TRUNCATE`
...
- [x] All geometries are `geography(MultiPolygon, 4326)`; pass `shapely.make_valid()` and `ST_IsValid` post-insert — *closed via M2-build operator pass 2026-06-23 (PR #74 / `a6cec6b`; env=dev / M2-build)*: all 197 CO geometry rows valid post-insert
...
- [x] Spatial queries (`extensions.ST_Covers` with `state='US-CO'`) against known coordinates return correct assignments — `spatial-test-points.json` verified — *closed via M2-build operator pass 2026-06-23 (env=dev / M2-build)*: 7-section spatial UAT all passed; WY negative-control = 0 rows (PRD 002 SC #4 confirmed)
- [x] Re-running ingestion is idempotent (UPSERT semantics — verified per S05.7 runbook with the post-E04 FK-cascade sequencing) — *closed via M2-build operator pass 2026-06-23 (env=dev / M2-build)*: UPSERT semantics verified per runbook protocol
