...
| Q19 | **Pre-M2 blocker** | id text-PK UPSERT slug-drift in 3 `db.py` helpers; ADR required; must land as a single PR before first year-over-year re-ingestion run — see §8 |
...
- **Q19 pre-M2 blocker** — Three helpers in `ingestion/ingestion/lib/db.py` can silently update slug-encoded fields on conflict via `ON CONFLICT DO UPDATE SET ...`:
  - `_UPSERT_SEASON_DEFINITION_SQL` (S03.7) — updates `name`, `weapon_type`, `residency` under the same `id`
  - `_UPSERT_LICENSE_TAG_SQL` (S03.7) — updates slug-encoded fields under the same `id`
  - `_UPSERT_REPORTING_OBLIGATION_SQL` (S03.9) — updates slug-encoded fields under the same `id`
