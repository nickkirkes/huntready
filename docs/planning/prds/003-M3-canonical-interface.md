...
- **Auth:** minimal but OAuth-2.1-*ready* for V1, and **split by client type** — because the data is public (no resource owner), the V1 job is access metering / abuse-control, not authorization. The **browser/public read path is no-BFF browser-direct (Q5), so it carries no secret token** — a browser-embedded bearer/API key would be exposed in client-side code and provide no real control; abuse-control there is **edge-side** (Cloudflare WAF + rate-limiting, optionally Turnstile). A **static bearer-token / API-key checkpoint** (Worker middleware validating `Authorization: Bearer <token>` against a Workers Secret) gates **programmatic / non-browser MCP clients** that can actually hold a secret (agentic clients, partner/CLI integrations), where it provides keying/metering. Both paths converge on the OAuth-2.1 upgrade path (`@cloudflare/workers-oauth-provider` — authorization-code + PKCE + RFC 7591 DCR) when a go-to-market strategy gives the auth model a subject to protect; it is *not* a V1 dependency. The spec makes authorization optional and directs stdio transports *not* to use it; a remote HTTP transport is what makes the authenticated-MCP spec exercisable at all.
...
- An OAuth-2.1-ready auth seam exists, split by client type: the public/browser read path is edge-rate-limited and carries no secret token; programmatic/non-browser clients pass a static bearer-token / API-key checkpoint. The integration point is structured for a later full-OAuth upgrade.
...
**Auth (split by client type, minimal for V1):** the **browser/public read path carries no secret token** (no-BFF browser-direct per Q5 — a client-embedded secret would be exposed; abuse-control there is **edge-side**: Cloudflare WAF + rate-limiting, optionally Turnstile). A **static bearer-token / API-key checkpoint** (Worker middleware) gates **programmatic / non-browser MCP clients** that can hold a secret, providing keying/metering. Both are architected behind a single auth seam so the full OAuth 2.1 flow (authorization-code + PKCE + DCR via `@cloudflare/workers-oauth-provider`) drops in later without rebuilding. The full, GTM-determined model is out of M3 scope.
...
**Exit criteria for E11:** The deployed server passes the M3 UAT criteria below; the token-gated programmatic path rejects an untokened request and admits a valid one, and the public browser path is edge-rate-limited (no browser-embedded secret); an induced error surfaces in the external capture service within 60s via the top-level error boundary; the README and the committed example client-config snippet let a fresh client connect; the M3→M4 handoff document lands; the `m3` tag is pushed.
...
9. On the **token-gated programmatic path**, a request without a valid token is rejected and one bearing a valid V1 bearer token is admitted. The **public browser-direct read path** carries no browser-embedded secret; its abuse-control is edge rate-limiting (verified by a rate-limit response under burst). (V1 depth — full OAuth 2.1 authorization-code is not required for M3 sign-off.)
...
**R3 — Auth depth vs. GTM uncertainty, and the browser-token exposure trap.** The production auth model is GTM-determined and TBD. Building too much auth now risks rework when the GTM strategy lands; building too little undershoots the authenticated-MCP goal. A specific trap: with no-BFF browser-direct calls (Q5), a static bearer/API key shipped to the browser is **exposed in client-side code** and provides no real control — so it must *not* be the browser path's abuse-control. Mitigation: the V1 decision is "auth-ready/minimal," **split by client type** — the browser/public read path carries no secret and is abuse-controlled at the **edge** (Cloudflare WAF + rate-limiting, optionally Turnstile); a static bearer-token / API-key checkpoint gates only **programmatic/non-browser clients** that can hold a secret; both sit behind one auth seam so the full OAuth 2.1 flow drops in later. On public data the V1 gate's purpose is metering/abuse-control, not authorization; the full model is an explicit deferred open question, not an M3 deliverable.
...
- **Auth is OAuth-2.1-ready but minimal for V1, split by client type.** The public/browser read path carries no secret token (no-BFF browser-direct per Q5; a client-embedded secret would be exposed) and is abuse-controlled at the edge (Cloudflare WAF + rate-limiting). A static bearer-token / API-key checkpoint gates programmatic/non-browser clients that can hold a secret. The full OAuth flow (via `@cloudflare/workers-oauth-provider`) is the documented upgrade path; the full GTM-determined model is deferred (Q22).
...
- The auth seam: M4's web client uses the public browser-direct read path (edge-rate-limited, no browser-embedded secret); the token-gated path serves programmatic clients. The seam is ready to deepen (full OAuth 2.1) when the GTM auth model lands.
