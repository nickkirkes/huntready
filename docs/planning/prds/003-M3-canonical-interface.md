...
- **Auth:** OAuth-2.1-*ready* but **unenforced in V1**. The data is public (no resource owner), so the V1 endpoint is a **single open, read-only MCP endpoint** with **no enforced authentication** — consistent with the standing V1 scope ("no rate limiting, authentication, billing, or tiered access" — see `architecture.md` § "What's out of scope"). The OAuth-2.1 **auth seam is wired as one middleware integration point but is not enforced** in V1 — it is the documented drop-in for real authentication (`@cloudflare/workers-oauth-provider` — authorization-code + PKCE + RFC 7591 DCR) when a go-to-market strategy gives the auth model a subject to protect (Q22). V1 does **not** rely on a static bearer token as an enforcement boundary: a single open endpoint cannot enforce a client-type split (a programmatic client could simply omit the token and take the open path), so any *enforced* token, tier, or quota requires a real boundary — a separate authenticated route or Cloudflare Access — which is a Q22/V2 decision. Baseline abuse protection in V1 is **Cloudflare's ambient DDoS/WAF** (a platform default, not a configured feature); configured rate-limiting is V2. The spec makes authorization optional and directs stdio transports *not* to use it; a remote HTTP transport is what makes the authenticated-MCP spec exercisable when the seam is later enforced.
...
- The deployed endpoint is an open, read-only public MCP endpoint with no enforced V1 authentication; the OAuth-2.1 auth seam is wired as one middleware integration point but unenforced — ready to deepen to real auth later (Q22). Baseline abuse protection is Cloudflare's ambient DDoS/WAF.
...
**Auth (open endpoint + wired-but-unenforced seam, minimal for V1):** the deployed MCP endpoint is **open and read-only** (public data); V1 enforces **no authentication** (consistent with the standing "no authentication in V1" scope). The OAuth-2.1 auth seam is **wired as a single middleware integration point but unenforced** — the drop-in for later real auth (authorization-code + PKCE + DCR via `@cloudflare/workers-oauth-provider`). **No static bearer token is relied on as a V1 enforcement boundary** — a single open endpoint can't enforce a client-type split (a client could omit the token and take the open path), so any enforced token/tier needs a real boundary (a separate authenticated route, or Cloudflare Access), which is a Q22/V2 decision. Baseline protection is Cloudflare's ambient DDoS/WAF; configured rate-limiting, enforced auth, tiering, and billing are V2.
...
- **Caching strategy tuning, query-performance tuning, configured rate-limiting / quota design.** Hyperdrive caching is used at conservative defaults and V1 relies on Cloudflare's ambient DDoS/WAF; tuning caching and *configuring* rate-limiting/quotas is V2. (Rate-limit *policy* is GTM-adjacent and deferred with the auth model.)
...
**Exit criteria for E08:** The server starts locally and deploys to a Workers preview; the MCP Inspector connects over Streamable HTTP; the driver spike is concluded and recorded; a read-only-enforced read against Supabase succeeds from the edge runtime (and a write attempt is provably rejected); CORS preflight succeeds from a test browser origin; the auth-seam middleware is present and, with the seam enabled in test mode, rejects a request lacking the configured credential and admits a valid one (the deployed V1 config leaves it unenforced — open endpoint); any seam credential lives in Workers Secrets, not committed; the Shape C envelope type compiles and round-trips a hand-constructed fixture; `mcp-server/` imports nothing from `ingestion/` (verified). No `any` types in serving TypeScript. (Any internal health check used here is not registered as a public MCP tool, so UAT criterion #1's "exactly five tools" holds.)
...
**Exit criteria for E11:** The deployed server passes the M3 UAT criteria below; the auth-seam middleware exists and (with the seam enabled in test mode) rejects a request lacking the configured credential and admits a valid one, while the V1 deployed endpoint serves open public reads under Cloudflare's ambient DDoS/WAF; an induced error surfaces in the external capture service within 60s via the top-level error boundary; the README and the committed example client-config snippet let a fresh client connect; the M3→M4 handoff document lands; the `m3` tag is pushed.
...
9. The **auth seam exists as a single middleware integration point**: a test with the seam enabled rejects a request lacking the configured credential and admits one bearing it (proving the OAuth drop-in point is real). In the V1 **deployed** configuration the endpoint is **open and read-only** (public data, no enforced authentication); Cloudflare's ambient DDoS/WAF protection is active. (V1 depth — full OAuth 2.1 enforcement is not required for M3 sign-off.)
...
**R3 — Auth depth vs. GTM uncertainty, and the open-endpoint enforceability trap.** The production auth model is GTM-determined and TBD. Building too much auth now risks rework when the GTM strategy lands; building too little undershoots the authenticated-MCP goal. Two related traps make a V1 token gate illusory: (1) with no-BFF browser-direct calls (Q5), a static token shipped to the browser is **exposed in client-side code**; (2) on a **single open endpoint** a token can't be enforced anyway — a programmatic client could just omit it and take the open path. Mitigation: V1 is "auth-ready/minimal" honestly — a **single open, read-only endpoint** with **no enforced auth** (public data), the OAuth-2.1 seam **wired but unenforced** (the drop-in point), and **Cloudflare's ambient DDoS/WAF** as baseline protection. Any *enforced* token, tier, quota, or authentication is deferred to Q22/V2 and would require a real boundary (a separate authenticated route, or Cloudflare Access). The full model is an explicit deferred open question, not an M3 deliverable.
...
- **Auth is OAuth-2.1-ready but unenforced in V1.** The endpoint is open and read-only (public data; no enforced authentication, consistent with the standing V1 scope). The auth seam is a wired-but-unenforced single middleware integration point (drop-in for OAuth via `@cloudflare/workers-oauth-provider`). No static token is relied on as a V1 boundary (unenforceable on a single open endpoint). Baseline protection is Cloudflare's ambient DDoS/WAF; enforced auth, tiering, and configured rate-limiting are V2 (Q22), requiring a real boundary (separate route or Cloudflare Access).
...
- The auth seam: M4's web client uses the open public read endpoint (no secret); the seam is wired-but-unenforced and ready to deepen to real auth (full OAuth 2.1, and any enforced token/tier behind a real boundary) when the GTM auth model lands.
...
- **Not designing configured rate-limiting / quota / tiering.** V1 relies on Cloudflare's ambient DDoS/WAF protection (a platform default); configured rate-limiting and quota policy are GTM-adjacent and deferred to V2.
