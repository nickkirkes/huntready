...
**Deployment:** A deployed remote server on Cloudflare Workers, reachable over HTTPS, with a working `.mcp-config.json` at repo root and `mcp-remote`-based local-dev instructions. Deployment is in M3 scope precisely because the server is *remote* — there is no longer a "local only in M3, deploy in M4" split for the MCP server itself. (The web companion's deployment remains M4.)
...
5. `get_tag_requirements(state, species, year, residency)` returns the tag type(s), draw vs. general status, application deadline(s), and embedded `draw_spec` context for a representative **Montana** draw hunt, with `sources[]`. (**N/A until `m2`:** for a Colorado preference-point hunt, the embedded `draw_spec` shows `point_system.kind='preference_linear'` and a populated `allocation_pool[]`.)
...
12. A fresh MCP client (Claude Desktop via `mcp-remote`, or a remote MCP client pointed at the HTTPS endpoint) can connect using the committed `.mcp-config.json` / README instructions and successfully invoke `get_regulations`.
...
- A documented deployment pattern (Cloudflare Workers) and `.mcp-config.json` M4's deployment config builds alongside.
