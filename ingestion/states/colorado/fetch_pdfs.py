"""Colorado PDF fetch orchestrator.

Reads ingestion/states/colorado/sources.yaml and invokes
...
Run from the repo root:
    ingestion/.venv/bin/python ingestion/states/colorado/fetch_pdfs.py
...
_SOURCES_YAML = _REPO_ROOT / "ingestion" / "states" / "colorado" / "sources.yaml"
_FIXTURE_DIR = _REPO_ROOT / "ingestion" / "states" / "colorado" / "fixtures"
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Fetch all Colorado Parks & Wildlife regulation PDFs listed in sources.yaml.",
    )
    parser.add_argument(
        "--sources",
        type=Path,
        default=_SOURCES_YAML,
        help="Path to sources.yaml (default: ingestion/states/colorado/sources.yaml)",
    )
    parser.add_argument(
        "--fixture-dir",
        type=Path,
        ...
        help="Output directory for PDFs and manifests "
             "(default: ingestion/states/colorado/fixtures/)",
    )
